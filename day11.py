...
def part1(gol):
    ...
    h = len(gol)
    while True:
        next = copy.deepcopy(gol)
        for x in range(w):
            for y in range(h):
                if gol[y][x] == ".":
                    continue
                n = neighbors(gol, x, y, w, h)
                if gol[y][x] == "L":
                    if n == 0:
                        next[y][x] = "#"
                if gol[y][x] == "#":
        ...
        print("\n".join("".join(row) for row in gol))
        print()
        if next == gol:
            break
        gol = next
    return sum(x == "#" for row in gol for x in row)

def trace_in_dir(gol, x, y, dx, dy):
    def ok(x, y):
        if x < 0 or y < 0:
            return False
        if y >= len(gol):
...
def build_adj(gol):
    ...
    for y in range(h):
        for x in range(w):
            for dx, dy in offs:
                adj = trace_in_dir(gol, x, y, dx, dy)
                if adj is None:
                    continue
                map[(x, y)].append(adj)
    return map

def neighbors2(gol, x, y, fwd_map):
    n = 0
    for xn, yn in fwd_map[(x, y)]:
        n += gol[yn][xn] == "#"
    return n

def part2(gol):

    fwd_map = build_adj(gol)
...
test_inp = [list(x) for x in test_inp]
